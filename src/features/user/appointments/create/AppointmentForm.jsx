...
export default function AppointmentForm() {
  ...
  const initialDate = location.state?.date; // 👈 primero obtenés esto

  const { clients, loadingClients, errorClients } = useClients();
  const { services, loadingServices, errorServices } = useServices();

  // 👇 ahora sí lo pasás al hook
  const { form, setForm, loading, handleSubmit } =
    useAppointmentCreate(initialDate);
